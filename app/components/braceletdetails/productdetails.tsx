...
const BraceletDetails = ({ data }: BraceletDetailsProps) => {
      ...
      <div className="gap-8 max-w-[1024px] m-auto px-4 sm:px-6 md:px-0 grid grid-cols-1 md:grid-cols-2">
        ...
        <div>
          ...
          <div className="h-[450px] flex items-center mb-8">
            {data.image && data.image[index] ? (
              <Image
                ...
                alt={data.image[index]?.alt || 'Bracelet Image'}
                height={450}
                width={450}
          ...
          <div className="flex gap-3 justify-center flex-wrap">
            {data.image?.slice(1).map((img, i) => (
              <div key={i} className="w-1/3 sm:w-1/3 md:w-1/4 lg:w-1/3 xl:w-1/4">
                <Image
                  loader={() => urlFor(img.asset).url()}
                  src={urlFor(img.asset).url()}
                  alt={img?.alt || 'Bracelet Image'}
                  height={400}
                  width={400}
        ...
        <div className="flex flex-col gap-6">
          ...
          <button
            ...
            onClick={() => addProduct(data as Product, qty)} 
          >
            Add to Cart
          </button>
        </div>
