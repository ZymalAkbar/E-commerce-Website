...
async function Earring() {
const xyz: Iearring[] = await client.fetch(`*[_type == 'earring']`);
  return (
    <>
    <Navbar />
      {/* HERO SECTION */}
      <div className="relative h-[500px] bg-transparent overflow-hidden">
        ...
        <div className="relative z-20 flex items-center justify-center h-full">
          <div className="text-center text-white">
            <h1 className="text-4xl font-bold">All Earrings</h1>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {xyz.map((data: Iearring, index: number) => {
            return (
              <div key={index} className="bg-pink-50 p-6 sm:p-8 lg:p-10 rounded-lg">
                <Link href={`/earrings/${data.slug.current}`}>
